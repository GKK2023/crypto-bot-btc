"""
CryptoBot - Spot Trading Bot BTC/USDT
...
# Trailing stop (vente automatique si le prix redescend de X% depuis le peak)
TRAILING_STOP_PCT = 0.2

# Seuil RSI pour achat
RSI_BUY_THRESHOLD = 40

# Prix d'achat manuel (optionnel — laisse vide pour auto-détection)
# Utilise-le si l'API Gate.io ne parvient pas à lire l'historique
MANUAL_ENTRY_PRICE = os.getenv('MANUAL_ENTRY_PRICE', '')
...
class SimpleHandler(BaseHTTPRequestHandler):
    def do_GET(self):
        ...
        self.end_headers()
        response = "<!DOCTYPE html><html><head><title>CryptoBot</title></head><body><h1>Bot BTC/USDT Active</h1></body></html>"
...
class SimpleBot:
    def __init__(self):
        print(f"[DEBUG] Bot BTC - __init__ appele")
        if PAPER_MODE:
            ...
            try:
                ...
                print("Connexion a Gate.io BTC reussie!")
            except Exception as e:
                print(f"Erreur de connexion: {e}")
                sys.exit(1)

        self.balance = self.get_real_balance()
        print(f"[DEBUG] Solde: USDT={self.balance.get('USDT', 0)}, BTC={self.balance.get('BTC', 0)}")

        btc_balance = float(self.balance.get('BTC', 0))
        if btc_balance >= MIN_POSITION_THRESHOLD:
            entry_price = None

            # 1. Priorité: prix manuel (ENV)
            manual = os.getenv('MANUAL_ENTRY_PRICE', '')
            if manual:
                try:
                    entry_price = float(manual)
                    print(f"[INFO] Prix d'achat manuel: ${entry_price:.4f}")
                except:
                    pass

            # 2. Sinon: récupérer depuis les trades
            if not entry_price:
                entry_price = self.get_entry_price_from_trades()

            # 3. Sinon: récupérer depuis les ordres
            if not entry_price:
                entry_price = self.get_entry_price_from_orders()

            # 4. Dernier recours: prix spot actuel
            if entry_price:
                self.position = {'side': 'long', 'entry': entry_price, 'amount': btc_balance, 'peak_price': entry_price}
                print(f"Position BTC: {btc_balance} @ ${entry_price:.4f}")
            else:
                current_price = self.get_price()
                if current_price:
                    self.position = {'side': 'long', 'entry': current_price, 'amount': btc_balance, 'peak_price': current_price}
                    print(f"[WARN] Prix d'achat non trouve -- spot utilise: ${current_price:.4f}")
                else:
                    self.position = None
        else:
            print(f"Pas de position BTC")
            self.position = None
    ...
    def get_entry_price_from_trades(self):
        """Récupère le PRIX DE FILL RÉEL du dernier achat sur Gate.io"""
        try:
            trades = self.exchange.fetch_my_trades(SYMBOL, limit=20)
            if not trades:
                print(f"[WARN] Aucun trade trouvé dans l'historique Gate.io")
                return None
            print(f"[DEBUG] {len(trades)} trades récupérés, recherche buys...")
            # Prendre le DERNIER trade d'achat (plus récent)
            buy_trades = [t for t in trades if t['side'] == 'buy']
            print(f"[DEBUG] {len(buy_trades)} buys trouvés dans les 20 derniers trades")
            if not buy_trades:
                # Log les derniers trades pour debug
                for t in trades[:3]:
                    print(f"  -> trade: side={t['side']}, price={t.get('price')}, ts={t.get('timestamp')}")
                return None
            # Trier par timestamp DESC = le plus récent en premier
            buy_trades.sort(key=lambda x: x.get('timestamp', 0), reverse=True)
            last_buy = buy_trades[0]
            # Utiliser 'average' si disponible (prix moyen réel du fill)
            price = last_buy.get('average') or last_buy.get('price')
            if price and float(price) > 0:
                print(f"[DEBUG] Prix fill reel: ${float(price):.4f} | qty: {last_buy.get('amount', 0)} | ts: {last_buy.get('timestamp')}")
                return float(price)
            return None
        except Exception as e:
            print(f"[DEBUG] Erreur fetch_my_trades: {e}")
            return None
        ...
        try:
            ...
            if isinstance(balance, dict):
                ...
                if isinstance(total, dict):
                    ...
                    btc_balance = float(total.get('BTC', 0) or 0)
    ...
    def calculate_profitability(self, current_price):
        try:
            ...
            amount_btc = float(self.position.get('amount', 0))
            if entry_price == 0 or amount_btc == 0:
            ...
            profit_usdt = (current_price - entry_price) * amount_btc
    ...
    def should_sell(self, data):
        try:
            ...
            # Met à jour le peak price
            if self.position and 'peak_price' in self.position:
                if current_price > self.position['peak_price']:
                    self.position['peak_price'] = current_price
                    print(f"    -> New peak: ${current_price:,.2f}")

            is_profitable, profit_pct, details = self.calculate_profitability(current_price)

            # --- VENTE SI TAKE-PROFIT ATTEINT ---
            if profit_pct >= MIN_PROFIT_THRESHOLD and profit_pct > 0:
                print(f" -> Vente TAKE-PROFIT: {profit_pct:.2f}% (+{details.get('profit_usdt', 0):.2f}$)")
                return True

            # --- TRAILING STOP: vente si le prix redescend de X% depuis le peak ---
            if self.position and 'peak_price' in self.position:
                peak = self.position['peak_price']
                drawdown = ((peak - current_price) / peak) * 100
                trailing_trigger = peak * (1 - TRAILING_STOP_PCT / 100)
                if current_price <= trailing_trigger and profit_pct > 0:
                    print(f" -> Vente TRAILING STOP: -{drawdown:.2f}% depuis ${peak:,.2f} | profit: {profit_pct:.2f}%")
                    return True

            # --- En attente ---
            peak = self.position.get('peak_price', details.get('entry_price', 0)) if self.position else 0
            target = details.get('target_price', 0)
            entry = details.get('entry_price', 0)
            print(f" -> En attente: {profit_pct:+.2f}% | Achat: ${entry:,.2f} | Cible: ${target:,.2f} | Peak: ${peak:,.2f}")
            return False
        except Exception as e:
            ...
            if usdt_to_use > 5:
                ...
                if amount_after_fee * price >= 7:
                    ...
                    if PAPER_MODE:
                        ...
                        self.position = {'side': 'long', 'entry': price, 'amount': amount, 'peak_price': price}
                        print(f"ACHAT simule: {amount:.6f} BTC @ ${price}")
                    else:
                        order = self.exchange.create_order(SYMBOL, 'market', 'buy', usdt_to_use)
                        # Extraire le prix de fill RÉEL depuis la réponse de l'ordre
                        fill_price = None
                        if order:
                            fill_price = order.get('average') or order.get('price')
                        if not fill_price:
                            fill_price = self.get_entry_price_from_trades()
                        if not fill_price:
                            fill_price = price
                        print(f"ACHAT reel: {amount:.6f} BTC @ ${fill_price:.4f} (fill) | prix spot: ${price:.4f}")
                        self.position = {'side': 'long', 'entry': fill_price, 'amount': amount, 'peak_price': fill_price}
        except Exception as e:
            ...
            if btc_balance >= MIN_POSITION_THRESHOLD:
                ...
                if amount * price >= 7:
                    if PAPER_MODE:
                        ...
                        print(f"VENTE simulee: {amount:.6f} BTC @ ${price}")
                        self.position = None
                    else:
                        order = self.exchange.create_order(SYMBOL, 'market', 'sell', amount)
                        print(f"VENTE reelle: {amount:.6f} BTC @ ${price}")
                        self.position = None
        except Exception as e:
    ...
    def run(self):
        ...
        print(f"MACD: False | Cooldown: 15 min | Alloc: {MAX_USDT_PERCENT}%")
        print(f"============================\n")
        while True:
            try:
                if not PAPER_MODE:
                ...
                if data is not None:
                    ...
                    if price is not None:
                        ...
                        print(f" USDT: {float(self.balance.get('USDT', 0)):.2f} | BTC: {float(self.balance.get('BTC', 0)):.6f}")
                        btc_balance = float(self.balance.get('BTC', 0))
                        if self.position is None:
                            ...
                            if btc_balance < MIN_POSITION_THRESHOLD:
                                print(f" -> Dust ignore: {btc_balance:.8f} BTC")
                                self.position = None
                            else:

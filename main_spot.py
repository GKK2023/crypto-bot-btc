# CryptoBot BTC/USDT - Spot Trading v2
# Corrections: trailing stop 0.2%, fill price = prix moyen pondéré position réelle
...
MIN_USDT_RESERVE = 5
MAX_USDT_PERCENT = 60       # BTC: 60% du solde (le plus gros allocation)
MIN_PROFIT_THRESHOLD = 0.5
TAKE_PROFIT_THRESHOLD = 0.5
TRAILING_STOP_PCT = 0.2
RSI_BUY_THRESHOLD = 40      # BTC: plus prudent, seuil plus bas
MACD_CONFIRM = False
# Pour BTC: le MACD est souvent negatif sur les timeframe courts
# On confirme avec MACD > Signal (cross haussier), sans exiger MACD > 0
...
class SimpleBot:
    def __init__(self):
        print(f"[DEBUG] Bot BTC - __init__ appelé")
        if PAPER_MODE:
    ...
    def _detect_existing_position(self):
        ...
        if btc_balance < MIN_POSITION_THRESHOLD:
            print(f"Pas de position BTC détectée")
            self.position = None
            return
        ...
        if not entry_price:
            ...
            print(f"[WARN] Impossible de trouver le prix d'achat — utilisation du prix actuel ${current:,.2f}")
            entry_price = current
        ...
        if entry_price:
            ...
            print(f"Position BTC détectée: {btc_balance:.8f} @ ${entry_price:,.2f}")
        else:
            self.position = None
    ...
    def get_entry_price_from_orders(self):
        """
        Récupère le prix moyen du dernier ordre BUY fermé (le plus fiable).
        """
        try:
            ...
            # average = prix moyen réel du fill (plus fiable que 'price')
    ...
    def get_entry_price_from_trades(self):
        """
        Calcule le prix moyen pondéré de la position BTC actuelle.
        Parcourt les trades BUY du plus récent au plus ancien jusqu'à
        atteindre (ou dépasser) le solde BTC actuel — ainsi on obtieni
        le vrai prix moyen d'ACHAT de la position actuellement détenue.
        """
        try:
            btc_balance = float(self.balance.get('BTC', 0))
            if btc_balance < MIN_POSITION_THRESHOLD:
            ...
            # Additionner les trades BUY jusqu'à atteindre le solde actuel
            for t in buy_trades:
                price = float(t.get('average') or t.get('price') or 0)
                ...
                total_btc += amount
                total_cost += cost
                if total_btc >= btc_balance * 0.95:
                    # On a atteint le solde actuel (tolérance 5%)
                    matched_cost = cost
                    break

            if total_btc > 0 and matched_cost > 0:
                avg_price = total_cost / total_btc
                print(f"[DEBUG] Prix moyen (pondéré): ${avg_price:,.2f} | BTC cumulé: {total_btc:.8f}")
                return avg_price
    ...
    def calculate_macd(self, data):
        try:
            ...
            macd = macd_vals[-1] if macd_vals else 0
            signal = self._ema(macd_vals[-9:] if len(macd_vals) >= 9 else macd_vals, 9)
    ...
    def should_sell(self, data):
        ...
        if self.position:
            if is_profitable and profit_pct > 0:
                print(f"  -> ✅ TAKE-PROFIT: {profit_pct:.2f}% (+{details.get('profit_usdt', 0):.2f}$ sur {details.get('amount', 0):.8f} BTC)")
    ...
    def buy(self):
        try:
            ...
            if PAPER_MODE:
                ...
                print(f"  ACHAT simulé: {amount:.8f} BTC @ ${price:,.2f}")
            else:
            ...
            try:
                ...
                if data is not None:
                    ...
                    if price is not None:
                        ...
                        btc_bal = float(self.balance.get('BTC', 0))
                        rsi, macd, signal, histogram, macd_improving = self.get_indicators(data)
                        ...
                        if self.position is None:
                            ...
                            if btc_bal < MIN_POSITION_THRESHOLD:
                                self.position = None
                            else:

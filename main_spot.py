...
class SimpleBot:
    def __init__(self):
        ...
        if btc_balance >= MIN_POSITION_THRESHOLD:
            ...
            if entry_price:
                self.position = {'side': 'long', 'entry': entry_price, 'amount': btc_balance}
                print(f"Position BTC: {btc_balance} @ ${entry_price:.4f}")
            else:
                current_price = self.get_price()
                if current_price:
                    self.position = {'side': 'long', 'entry': current_price, 'amount': btc_balance}
                    print(f"Position BTC au prix actuel: {btc_balance} @ ${current_price:.4f}")
                else:
                    self.position = None
        else:
            ...
            if usdt_to_use > 5:
                ...
                if amount_after_fee * price >= 7:
                    ...
                    if PAPER_MODE:
                        ...
                        print(f"ACHAT reel: {amount:.6f} BTC @ ${price}")
                        self.position = {'side': 'long', 'entry': price, 'amount': amount}
        except Exception as e:

...
class SimpleBot:
    def __init__(self):
        ...
        if PAPER_MODE:
            ...
            self.balance = {'USDT': 10000, 'BTC': 0}; self.position = None
        else:
    ...
    def _detect_existing_position(self):
        ...
        if btc_balance < MIN_POSITION_THRESHOLD: print(f"Pas de position BTC"); self.position = None; return
        ...
        if entry_price:
            self.position = {'side': 'long', 'entry': float(entry_price), 'amount': btc_balance}
            self.peak_price_since_buy = float(entry_price)
            print(f"Position BTC: {btc_balance:.8f} @ ${entry_price:,.2f}")
    ...
    def get_real_balance(self):
        try:
            ...
            if isinstance(balance, dict):
                ...
                if isinstance(total, dict): return {'USDT': float(total.get('USDT', 0) or 0), 'BTC': float(total.get('BTC', 0) or 0)}
            return {'USDT': 0, 'BTC': 0}
        except: return {'USDT': 0, 'BTC': 0}
    ...
    def buy(self):
        try:
            ...
            if PAPER_MODE:
                self.balance['USDT'] -= usdt_to_use; self.balance['BTC'] += amount
    ...
    def sell(self):
        try:
            ...
            if PAPER_MODE:
                self.balance['BTC'] = 0; self.balance['USDT'] += amount * price * (1 - TRADING_FEE)
            ...
            try:
                ...
                if data is not None:
                    ...
                    if price is not None:
                        ...
                        usdt_bal = float(self.balance.get('USDT', 0)); btc_bal = float(self.balance.get('BTC', 0))
                        rsi, macd, signal, histogram, macd_improving = self.get_indicators(data)
                        ...
                        if self.position is None:
                            ...
                            if btc_bal < MIN_POSITION_THRESHOLD: self.position = None
